...
Image size = 332
'''
#This is done in order to transform a set of images into a set of tensors with variables (height, width, RGB).
data_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225])])
dataset_3 = datasets.ImageFolder(root = 'Train_new_1', transform = data_transform) 
test_3 = datasets.ImageFolder(root = 'Test_new_1', transform = data_transform) 

train_loader = DataLoader(dataset_3, batch_size = 32, shuffle = True)
test_loader = DataLoader(test_3, batch_size = 32, shuffle = True)

#    Xn = Xn.transpose(1, 2).transpose(2, 3).numpy()

#for X, y in train_loader:
#    Xn = X[0].view(3, 332, 332)
#    Xn = Xn.transpose(0, 1).transpose(1, 2).numpy()
#    Xn = Xn.astype('float64')
#    Xn = (Xn - np.min(Xn)) / (np.max(Xn) - np.min(Xn))
#    Xn = np.rint(Xn * 255).astype('uint8')
#    to_pil_image = transforms.ToPILImage()
#    results = to_pil_image(Xn)
#    results.show()
#    print(y)
#    break
...
class ConvClassifier1(nn.Module): #Define the neural network (nn)
    def __init__(self, image_size): #Upload the image
        ...
        self.linear_layers = nn.Sequential(nn.Linear(image_size//2*image_size//2*16, 2), #10 classes of numbers
                                           nn.LogSoftmax(dim=1))
...
def train(network, epochs, learning_rate):
    loss = nn.NLLLoss() #Initialises the loss parameter, defined as a Negative Log Likelihood function (google it)
    ...
    try:
        for epoch in range(epochs): #For each epoch in range that we have defined.
            losses = []
            accuracies = []
            for X, y in train_loader:
                network.zero_grad() #Sets gradients calculated before to zero to allow back-propagation
                prediction = network(X) #Array of images is uploaded into NN. Then NN returns its prediction on wheights. 
                loss_batch = loss(prediction, y) #Returns a loss on the batch based on predictions and true labels. 
            ...
            train_loss_epochs.append(np.mean(losses)) #He took the average of the values so the graph looks pretty.
            train_accuracy_epochs.append(np.mean(accuracies))#Otherwise, there is too much noise. Myself, I would include the noise. It looks a bit more scientific. 
            #Testing section is below.
            losses = []
            accuracies = []    
            ...
            plt.legend(loc=0, fontsize=13)
            plt.grid()
            plt.savefig('Output_pion8_new.png', dpi = 250)
            plt.show()
    except KeyboardInterrupt: #is needed so you could stop the learning process with your progress saved. 
        pass

neural_network_1 = ConvClassifier1(332) # Created neural network
train(neural_network_1, epochs=5, learning_rate=0.001)
